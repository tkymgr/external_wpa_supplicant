                          ...
                          struct wpa_scan_result *bss, struct wpa_ssid *ssid)
{
    ...
    if (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
        ...
        ret = wapi_conf_read_certfile((const char *)ssid->as_cert_uri, NULL, ap_param.para.as);
        if (ret < 0) {
	    wpa_msg(wpa_s, MSG_INFO, "WPA: Cert checking failed - " "cert files may be incorrect");
            wpa_printf(MSG_ERROR, "ERROR wapi_conf_read_certfile %s", ssid->as_cert_uri);
            return;
        }
        ret = wapi_conf_read_certfile((const char *)ssid->user_cert_uri, ssid->user_key_uri, ap_param.para.user);

        if (ret < 0) {
	    wpa_msg(wpa_s, MSG_INFO, "WPA: Cert checking failed - " "cert files may be incorrect");
            wpa_printf(MSG_ERROR, "ERROR wapi_conf_read_certfile %s", ssid->user_cert_uri);
            return;
        }
    } else {
...
#ifdef ATHEROS_WAPI
            wpa_drv_set_port_state(wpa_s, 1);
#endif
        }
    }
}
...
#define KEYSTORE "keystore://"
    ...
    return keystore_get(&key[11], 12, value);
}

int wapi_conf_read_certfile(const char *cert_file, const char *cert_key, unsigned char *cert_buf)
{
    if (NULL == cert_file || NULL == cert_buf) {
         wpa_printf(MSG_ERROR, "Error: try to open a null certificate or cert_buf is null");
         return -1;
    }

    wpa_printf(MSG_INFO, "reading cert %s\n", cert_file);
    int length = wapi_get_fromKeyStorage((const char *)cert_file, (char *)cert_buf);
    if (length < 0) {
         wpa_printf(MSG_ERROR, "Error reading cert. storage for cert file");
         perror("Open cert. storage");
         return -1;
     }
     if (NULL != cert_key) {
         length = wapi_get_fromKeyStorage(cert_key, &cert_buf[length]);
         if (length < 0) {
               wpa_printf(MSG_ERROR, "Error reading cert. storage for private key");
               perror("Open private key in cert storage");
               return -1;
         }
    }
    return 0;
